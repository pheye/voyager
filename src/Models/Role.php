...
class Role extends Model
{
    ...
    {                                        
        return $this->belongsToMany(Voyager::modelClass('Policy'))->withPivot('value');                   
    } 
    ...
    {                                        
        ...
        if ($cache === null)                 
            throw new \ErrorException("{$this->name}:role usage cache is null");  
        foreach($this->policies as $policy) {
            $key = $policy->key;             
            $right = [$policy->type, $policy->pivot->value];                              
            if (!isset($cache[$key]) || $cache[$key] != $right)                           
                throw new \ErrorException("{$this->name}:$key should be " . json_encode($right) . "but result is " . (isset($cache[$key]) ? json_encode($cache[$key]) : " not set"));                                     
        }                                    
        return true;                         
    }                                        
